...
@task(category="data_platform_breach")
def data_platform_breach_suite():
    """
    One Task covering all data_platform_breach samples.
    Each sample dynamically loads its own tools based on metadata.
    """
    TOOL_REGISTRY = get_tool_registry("data_platform_breach")
    return Task(
        dataset=get_dataset("data_platform_breach"),
