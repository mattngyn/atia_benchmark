...
@task(category="identity_access_mgmt_abuse")
def identity_access_mgmt_abuse_suite():
    """
    One Task covering all identity_access_mgmt_abuse samples.
    Each sample dynamically loads its own tools based on metadata.
    """
    TOOL_REGISTRY = get_tool_registry("identity_access_mgmt_abuse")
    return Task(
        dataset=get_dataset("identity_access_mgmt_abuse"),
