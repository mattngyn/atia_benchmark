...
@task(category="customer_support_crm_abuse")
def customer_support_crm_abuse_suite():
    """
    One Task covering all customer_support_crm_abuse samples.
    Each sample dynamically loads its own tools based on metadata.
    """
    TOOL_REGISTRY = get_tool_registry("customer_support_crm_abuse")
    return Task(
        dataset=get_dataset("customer_support_crm_abuse"),
