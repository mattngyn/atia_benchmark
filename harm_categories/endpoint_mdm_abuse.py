...
@task(category="endpoint_mdm_abuse")
def endpoint_mdm_abuse_suite():
    """
    One Task covering all endpoint_mdm_abuse samples.
    Each sample dynamically loads its own tools based on metadata.
    """
    TOOL_REGISTRY = get_tool_registry("endpoint_mdm_abuse")
    return Task(
        dataset=get_dataset("endpoint_mdm_abuse"),
